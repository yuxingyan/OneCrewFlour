...
public class FlourprocessAction extends ActionSupport {
	...
	public String addflourprocess() throws ParseException{
		...
		String s3=ServletActionContext.getRequest().getParameter("option");
		
		process.setUserid(1);
	   process.setDate(date);
	    process.setType(s3);
	    Flourstore flourstore=flourstoreBiz.findByType("–°¬Û");
	    Flourstore flourstore2=flourstoreBiz.findByType(process.getType());
	    Flourstore flourstore3=flourstoreBiz.findByType("ÙÔ∆§");
		...
		flourstoreBiz.updateflourstore("–°¬Û", flourstore.getMillweight()-process.getRawweight());
		  Checkstore checkstore1=checkflourstoreBiz.findByName("–°¬Û");
		  ...
		  if(flourstore3.getMillweight()!=null){
			  flourstoreBiz.updateflourstore("ÙÔ∆§", flourstore3.getMillweight()+process.getBranweight());
			  Checkstore checkstore=checkflourstoreBiz.findByName("ÙÔ∆§");
			  ...
			  flourstoreBiz.addflourstore("ÙÔ∆§", process.getBranweight());
			  Checkstore checkstore=checkflourstoreBiz.findByName("ÙÔ∆§");
			  if(checkstore.getSumstore()==null){
					checkstore.setClientstore((float)0);
					checkstore.setSumstore(process.getFlourweight());
					checkstore.setType("ÙÔ∆§");
